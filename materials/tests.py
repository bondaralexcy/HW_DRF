...
class LessonTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        self.course = Course.objects.create(title='DJANGO DRF', description='DJANGO DRF', owner=self.user)
        self.lesson = Lesson.objects.create(title='Lesson_1', description='Lesson_1', course=self.course, owner=self.user)
    ...
    def test_update_lesson(self):
        ...
        self.assertEqual(
            response.data.get('title'), 'New_lesson'
        )
...
class SubscriptionTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(email='admin@sky.pro')
        self.course = Course.objects.create(title='DRF', description='DRF', owner=self.user)
        self.lesson = Lesson.objects.create(title='Lesson_1', description='Lesson_1', course=self.course, owner=self.user)
        self.subscription = CourseSubscription.objects.create(course=self.course, user=self.user)
        self.client.force_authenticate(user=self.user)
